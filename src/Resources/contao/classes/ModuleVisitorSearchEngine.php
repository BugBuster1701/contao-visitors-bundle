...
class ModuleVisitorSearchEngine // extends Frontend
{
	...
	protected function reset()
	{
		...
		{
			$this->_http_referer  = $_SERVER['HTTP_REFERER'] ?? self::REFERER_UNKNOWN;
		}
	...
	{
		if (
			preg_match('/(http|https):\/\/search\.avg\.com/', $this->_http_referer)
	...
	protected function checkEngineBaidu()
	{
		if (preg_match('/(http|https):\/\/www\.baidu\.com\/s/', $this->_http_referer))
		{
			...
			if (isset($this->_parse_result['wd']))
			{
	...
	{
		if (
			preg_match('/(http|https):\/\/suche\.web\.de\/search/', $this->_http_referer)
			...
			if (isset($this->_parse_result['su']))
			{
				$this->_keywords = $this->_parse_result['su'];
			}
			if (isset($this->_parse_result['q']))
			{
				$this->_keywords = $this->_parse_result['q'];
	...
	{
		if (
			preg_match('/(http|https):\/\/www\.lycos\.de/', $this->_http_referer)
			|| preg_match('/(http|https):\/\/search\.lycos\..*/', $this->_http_referer)
		) {
			$this->_search_engine = self::SEARCH_ENGINE_LYCOS;
			if (isset($this->_parse_result['query']))
			{
				$this->_keywords = $this->_parse_result['query'];
			}
	...
	{
		if (
			preg_match('/(http|https):\/\/.*\.aol\..*/', $this->_http_referer)
	...
	{
		if (
			preg_match('/(http|https):\/\/yandex\.ru\/yandsearch/', $this->_http_referer)
			...
			if (isset($this->_parse_result['text']) && '' !== $this->_parse_result['text'])
			{
	...
	protected function checkEngineBigfinder()
	{
		if (preg_match('/(http|https):\/\/.*\.bigfinder\.de\//', $this->_http_referer))
		{
			...
			if (isset($this->_parse_result['suchwert']))
			{
				$this->_keywords = $this->_parse_result['suchwert'];
			}
			if (isset($this->_parse_result['q']))
			{
				$this->_keywords = $this->_parse_result['q'];
	...
	protected function checkEngineKennenSieMich()
	{
		if (preg_match('/(http|https):\/\/kennensiemich\.ch\/roboto/', $this->_http_referer))
		{
			...
			if (isset($parse_result['q']))
			{
				$this->_keywords = $parse_result['q'];
			}
	...
	{
		if (
			preg_match('/(http|https):\/\/ecosia\.org\//', $this->_http_referer)
